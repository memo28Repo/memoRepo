...
export class Logs {
    ...
    responseSuc(response) {
        if (!Reflect.get(response.config, 'debugger'))
            return response;
        ...
        const getMethod = generateLogInformation.getMethods();
        const getURL = generateLogInformation.getURL();
        console.groupCollapsed(`%c Response %c ${getTime.time}%c ${getMethod.method}%c ${response.status + ':' + response.statusText} %c ${getURL.url} %c 执行耗时:${executionTime}ms`, `color: ${colors.requestResponseSucceeded};`, getTime.color, getMethod.color, 'color: black;', generateLogInformation.getParams(), 'color: #58a6ff');
